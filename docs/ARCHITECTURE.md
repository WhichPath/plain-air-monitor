...
- Flash-backed aggregate records store one aligned 10-minute bucket per record.
  Records store only the bucket `end_epoch_ms`; the start can be derived by
  subtracting 10 minutes. Each field stores avg/min/max/count. Long-term fields
  are PM2.5, PM10, SHT45 temperature, SHT45 humidity, BMP581 pressure, SCD41 CO2,
  SGP41 VOC Index, and SGP41 NOx Index.
